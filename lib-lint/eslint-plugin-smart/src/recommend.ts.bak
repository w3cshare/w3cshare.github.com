 ...
 * @LastEditTime: 2025-05-06 13:13:57
...
'require-yield': 'error',
  ...
  // for-in循环需要hasOwnProperty检查
  'no-async-promise-executor': 'error',

  // 禁止使用异步函数作为Promise executor
...
allowObjectEnd: false, 
      ...
      // 不允许在数组结束处的注释不需要前置空行
      allowClassStart: true, // 不允许在类结束处的注释不需要前置空行
...
// 禁止在对象实例上直接使用Object.prototype的内部属性
...
trailingComma: 'all',
  ...
  'unused-imports/no-unused-vars': [
    'error',
    {
      ...
      // 检查所有变量
      varsIgnorePattern: '^_', // 忽略以_开头的捕获错误
      /*
...
export const typescriptRules: ESLintRuleSet = {
  ...
  // 禁止定义未使用的变量（由unused-vars-and-imports插件处理）
  ...
  // 不要求导出函数和类的公共类方法的显式返回和参数类型
  '@typescript-eslint/no-non-null-assertion': 'warn',

  // 禁止使用any类型
  '@typescript-eslint/no-unused-vars': 'off', // 警告使用@ts-注释
...
'react/no-children-prop': 'error',
  ...
  // 安全警告：禁止不安全的target="_blank"
'react/no-direct-mutation-state': 'error',
  // 要求img标签有alt属性
  'jsx-a11y/anchor-has-content': 'error',

  
// 确保锚点标签<a>包含可访问的内容，避免空链接影响屏幕阅读器用户
...
// 验证所有aria-*属性都是有效的ARIA属性
  'jsx-a11y/aria-role': 'error',

  

// 验证role属性的值是有效的ARIA角色
...
// 避免使用accesskey属性，防止与屏幕阅读器快捷键冲突
  'jsx-a11y/anchor-is-valid': 'warn',


// 禁止在不支持ARIA的元素上使用ARIA属性
...
'react-hooks/exhaustive-deps': 'warn',
  ...
  // 确保标题标签(h1-h6)包含内容，避免空标题影响屏幕阅读器用户
  'jsx-a11y/html-has-lang': 'error',

  // 要求<html>标签有lang属性，声明页面语言
  'jsx-a11y/img-redundant-alt': 'warn',

  // 避免图片alt属性包含冗余文本(如图片/图像等)
  'jsx-a11y/no-access-key': 'warn',
  'react/jsx-closing-bracket-location': ['error', 'line-aligned'],

  
  // 强制组件props类型检查，帮助捕获类型错误，建议设为warn级别以便开发时提醒
...
'react/jsx-fragments': ['error', 'syntax'],
   ...
   */
  'react/react-in-jsx-scope': 'off', // 强制组件名使用PascalCase命名规范
}
...
// 防止<script setup>使用的变量<template>被标记为未使用
  ...
  // 强制Vue组件props类型定义，提高代码可维护性
'vue/no-reserved-component-names': 'error',
  
  // 禁止同时使用v-if和v-for
'vue/require-v-for-key': 'error',
  // 禁止使用Vue保留名称作为组件名，避免冲突
  'vue/no-unused-components': 'error',
  // 防止Vue JSX中未使用的变量引起错误
...
'vue/component-name-in-template-casing': [
    ...
    'kebab-case',
    {
      // 对所有组件生效
      ignores: [],
      registeredComponentsOnly: false, // 无例外情况
    },
  ],
...
export const nodejsRules: ESLintRuleSet = {
  ...
  // 允许使用现代ES语法（NestJS使用TypeScript编译）
  'node/no-missing-import': 'off',

  // TypeScript会处理导入，不需要Node.js的导入检查
  'node/no-unpublished-import': 'off',

  // 禁止使用__dirname或__filename做路径拼接
  'node/no-unsupported-features/es-syntax': 'off', // 允许导入devDependencies中的包（用于测试等）
}
...
export const jsonRules: ESLintRuleSet = {
  ...
  'jsonc/sort-keys': [
    ...
    {
      order: { type: 'asc' },
      pathPattern: '^(?:dev|peer|optional|bundled)?[Dd]ependencies$', // 依赖项按字母排序
    },
